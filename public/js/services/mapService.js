app.service('mapService', function ($http) {
  ...
  this.updateMap = function (map) {
    var map_id = map._id;
    delete map._id;
    return $http({method: 'PUT', url: 'http://192.168.137.10:5001/API/maps/'+map_id, data:map}).then(function(result) {
           return result.data;
    });
  };
  ...
  this.insertMap = function (options) {
    ...
    return $http({method: 'POST', url: 'http://192.168.137.10:5001/API/maps', data:map}).then(function(result) {
           return result.data;
    });
  };
